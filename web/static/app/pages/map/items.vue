        ...
        >
          ...
          <ul>
            ...
            >
              ...
              <div class="level">
                <template v-for="(level_format, index) in item.levels_format">
                  <div
                    v-if="level_format"
                    :key="index"
                    :class="`level-${index - 1}`"
                  >
                    <span>{{ level_format }}</span>
                  </div>
    ...
    categories_format() {
      return this.categories.map((categorie) => {
        categorie.items = categorie.items.map((item) => {
          ...
          item.levels_format = [1, 2, 3].map((level) => item.levels[level]);
          return item;
        });
        return categorie;
      });
