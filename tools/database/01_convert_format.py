...
"""
        ...
        for e in elems:
            ...
            cur_mod.execute(
                sql_elem, (err_id, num, e[0][0].upper(), elm_id, elm_tags, None)
            )
            num += 1
...
cur_mod.execute(
    """create table dynpoi_update_last as ( SELECT dynpoi_update.source, max(dynpoi_update."timestamp") AS "timestamp"
   FROM dynpoi_update
